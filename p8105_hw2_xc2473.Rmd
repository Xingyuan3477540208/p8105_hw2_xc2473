...
merge_1_data = left_join(pols_month_data, snp_data, by = "year", "month")

merge_data = left_join(unempolyment_data, merge_1_data, by = "year", "month")
```

The range of years in pols-month.csv document is `r range(pull(pols_month_data, year))`. The key variables are year, month, ... dimension???
The range of years in snp.csv document is `r range(pull(snp_data, year))`. The key variables are year, month, day and close.
The range of years in unemployment.csv is `r range(pull(unempolyment_data, year))`. The key variables are year, month, unemployment.
...
wnh_2016 = ggplot(pop_baby_name_data, aes(x = rank, y = count)) + geom_point()
```
